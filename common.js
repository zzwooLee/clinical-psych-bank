...
async function loadQuestions() {
    ...
    if(area) area.innerHTML = '<div class="loading">문제를 구성하는 중...</div>';
    ...
    try {
        ...
        if(area) area.innerHTML = '<div class="error">데이터 로드 실패: ' + error.message + '</div>';
    }
}
...
function renderQuestion() {
    ...
    if (!area || allQuestions.length === 0) {
        if(area) area.innerHTML = '<div class="card">해당 조건의 문제가 없습니다. 필터를 확인해주세요.</div>';
        return;
    }
...
function checkAnswer(selected) {
    ...
    btns.forEach(btn => btn.style.pointerEvents = 'none'); // 클릭 방지
...
// 페이지 로드 시 유저 정보가 있으면 UI 업데이트 (index.html에서 실행)
